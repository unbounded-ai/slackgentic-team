...
LOOP_SUMMARY_STATUS_ORDER = (
    "ok",
    "notable_positive",
    "found_issue",
    "found_very_severe_issue",
    "resolved",
    "action_taken",
    "failed",
)
LOOP_SUMMARY_STATUSES = frozenset(LOOP_SUMMARY_STATUS_ORDER)
LOOP_SUMMARY_STATUS_CHOICES = "|".join(LOOP_SUMMARY_STATUS_ORDER)
...
LOOP_SUMMARY_SIGNAL_EXAMPLE = json.dumps(
    {
        "status": LOOP_SUMMARY_STATUS_CHOICES,
...
def parse_agent_loop_summary_signal(signal: str) -> LoopSummaryParseResult:
    ...
    if status not in LOOP_SUMMARY_STATUSES:
        return LoopSummaryParseResult(
            error=f"loop summary status must be one of {', '.join(LOOP_SUMMARY_STATUS_ORDER)}"
        )
        ...
        [
            ...
            *(
                [
                    ...
                    "harness then posts no card. Every other status posts a card, so use "
                    "one only for something worth a notification, and then keep the report "
                    "short and specific: notable_positive for good news, found_issue for a "
                    "problem, found_very_severe_issue only when the owner should drop "
                    "everything now. When an issue you reported has cleared, use status "
                    "resolved, not found_issue.",
            ...
            "- status: ok (nothing notable), notable_positive (good news worth a card, such "
            "as a release reaching production), found_issue (anomaly or problem the owner "
            "should see), found_very_severe_issue (drop everything now: an outage or data "
            "loss in progress), resolved (an issue reported earlier has cleared), "
            "action_taken (you changed something), failed (you could not do the mission).",
